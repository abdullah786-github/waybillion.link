...
if ($user->_logged_in || !$system['system_public']) {
	header("Location: https://waybillion.link/pages/pageview/".$_GET['username']);
	user_access();
}
  ...
  switch ($_GET['view']) {
    case '':
      ...
      if ($system['colored_posts_enabled']) {
		...
		$categories = $user->get_categories_page("blogs_categories", 0, false, true,$spage['page_id']);
		$smarty->assign('categories', $categories);
      /* get posts */
      $posts = $user->get_posts(array('get' => 'posts_page', 'id' => $spage['page_id']));
      /* assign variables */
      $smarty->assign('posts', $posts);
	  $smarty->assign('get', 'posts_page');
      $smarty->assign('_id', $spage['page_id']);
	  $popular = $user->get_posts(array('get' => 'popular', 'id' => $spage['page_id']));
        /* assign variables */
        $smarty->assign('popular', $popular);
		  
		$discover = $user->get_posts(array('get' => 'discover', 'id' => $spage['page_id']));
        /* assign variables */
        $smarty->assign('discover', $discover);
		
		$trending = $user->get_trending_hashtags_page($spage['page_id']);
		//echo '<pre>';print_r($popular);
		/* assign variables */
		$smarty->assign('trending', $trending);

      $suggested_pages = $user->get_pages(array('page_id' => $spage['page_id']));
      $smarty->assign('suggested_pages', $suggested_pages);
      break;
