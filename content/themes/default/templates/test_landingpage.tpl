...
<div class="banner_slider_wrapper">
  ...
  {foreach $latest_articles as $article}
      ...
      <div class="video_btn_banner">
        <button type="button" class="video_popup_btn" data-toggle="modal" data-src="{$article['article']['cover_video']}">
          <span></span>
        </button>
      </div>
      {/if}
  ...
  {foreach $latest_articles as $article}
		<div><img src="{$article['article']['parsed_cover']}"></div>
  {/foreach}
	</div>
  </div>
        ...
        <ul>
          ...
          {foreach $categories as $category}
            <li style="display:inline;" data-val="{__($category['category_name'])}"> <a href="{$system['system_url']}/blogsfront/category/{$category['category_id']}/{$category['category_url']}">
               <div class="category_icon">{if $category['category_icon']} <img src="{__($category['category_icon'])}"  style="height:20px; width:20px;" /> {/if}<span>{__($category['category_name'])}</span></div>
            </a></li>
          {/foreach}
        </ul>
      </nav>
          ...
          {if $articles}
            <ul class="row">
              ...
              {foreach $articles as $article}
                {include file='_fees_article_test.tpl' _tpl="featured" _iteration=$article@iteration}
              {/foreach}
              <!-- articles -->
            </ul>
