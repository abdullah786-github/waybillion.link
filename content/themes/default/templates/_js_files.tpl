{strip}
  ...
  $('.send_otp').click(function() {
	...
	var myKeyVals = { phone : phone_code+phone_num};
	$.ajax({
		type: 'POST',
		url: ajax_path+"core/signin.php?send_otp=1",
		data: myKeyVals,
		dataType: "text",
		cache: false,
		success: function(html){
		if(html == 0){
			$(".alert.alert-success.mb0.x-hidden").hide();
			$(".alert.alert-danger.mb0.x-hidden").html("{__('Wrong Phone number')}");
			$(".alert.alert-danger.mb0.x-hidden").show();
		}else{
			$(".alert.alert-success.mb0.x-hidden").html("{__('OTP Sent')}");
			$(".alert.alert-success.mb0.x-hidden").show();
			$('#signmethod').attr("style", "pointer-events: none;");
			$('#phone_code').attr("style", "pointer-events: none;");
			$('#phone_num').attr("style", "pointer-events: none;");
			$('.send_otp').text("{__('RESEND')}");
			$('#otp').show();
			$('.login').show();
			$(".alert.alert-danger.mb0.x-hidden").hide();
		}
	  }
	});
	});
	$('input[name=otp]').keyup(function() {
        ...
        $('select[name=sign_method]').change(function() {
      var groupName = $(this).val();
        if (groupName == 'ByPhone') {
            $('#username_email').hide();
            $('#password').hide();
            $('input[name=username_email]').removeAttr('required');
            $('input[name=password]').removeAttr('required');
            $('#Phone').show();
			$('.send_otp').show();
			$('.login').hide();
			$('.login').prop( "disabled", true );
        } else if (groupName == 'ByEmail') {
            $('#Phone').hide();
            $('input[name=user_ph]').removeAttr('required');
            $('#username_email').show();
            $('#password').show();
			$('.send_otp').hide();
			$('#otp').hide();
       ...
       $('select[name=registration_method]').change(function() {
           $('#r_first_name').show();
                        $('#r_last_name').show();
                        $('#r_username').show();
      var groupName = $(this).val();
        if (groupName == 'ByPhone') {
            ...
            console.log("Else");
            $('#phone_code').removeAttr('required');
