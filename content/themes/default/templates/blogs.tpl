             ...
             <div class="tabs tabs-style-iconbox"> 
              <nav>
                <ul>
                  ...
                  {foreach $categories as $category}
                    <li style="display:inline;" data-val="{__($category['category_name'])}"> <a href="{$system['system_url']}/blogs/category/{$category['category_id']}/{$category['category_url']}">
                     <div class="category_icon">{if $category['category_icon']} <img src="{__($category['category_icon'])}"  style="height:35px; width:35px;" /> {/if}<span>{__($category['category_name'])}</span></div>
            </a></li>
                    </a></li>
                  {/foreach}
                </ul>
            ...
            <div class="col-12 offcanvas-mainbar">
              <div class="row">
                ...
                <div class="blog_uppercategory"> 
                  {foreach $categories as $category}
                     {if $category['category_name'] == $article['article']['category_name']}
                     <a class="article-category" href="https://waybillion.link/blogs/category/{$category['category_id']}/{$category['category_name']}">
                           
                      <img src="{$category['category_icon']}" style="width:35px;height:35px;" /> {$article['article']['category_name']} 
                          </a>
                      {/if}
                     {/foreach}
                      <select name="categories">
                    <option value="">{__("Browse other Categories")}</option>
                     {foreach $categories as $category}
                  ...
                  {foreach $latest_articles as $article}
                    {include file='__feeds_article_right.tpl' _small=true}
                  {/foreach}
                  <!-- read more -->
                </div>
                <!-- right panel -->
          ...
          {elseif $view == "article"}
            ...
            <div class="col-12 offcanvas-mainbar">
              <div class="row">
                <!-- left panel -->
                <div class="col-md-8 mb20">
                  <div class="article mb20 post" data-id="{$article['post_id']}">
                    <div class="article-wrapper {if $user->_logged_in}pb10{/if}">
                      ...
                      <!-- article category -->
                      <div class="mb10">
      				{foreach $article['article']['all_cats'] as $key=>$itemvar}
                        <a class="article-category" href="{$system['system_url']}/blogs/category/{$key}/{$itemvar}">
      						{$itemvar}
                        </a>
      				{/foreach}
                      </div>
                      ...
                      <!-- article title -->
                      <h2 class="article_title">{$article['article']['title']}</h2>
                      <!-- article title -->
                          ...
                          <ul>
                            {foreach $article['article']['parsed_tags'] as $tag}
                              <li>
                                ...
                                <a href="{$system['system_url']}/search/{$tag}/articles">{__($tag)}</a>
                              </li>
                            {/foreach}
                          </ul>
                        </div>
                ...
                <div class="js_ajax-forms-html " data-url="posts/article.php?do=edit&id={$article['post_id']}">
                  <div class="card-body">
                    ...
                    <div class="form-group form-row">
                        ...
                        <div class="col-md-10">
                            <label class="switch">
                              <input class="checkbox_cover" type="checkbox" value="{$article['article']['media_type']}" name="media_type">
                    ...
                    <div class="form-group form-row cover_video" style="display:none;">
                      ...
                      <div class="col-md-10">
                        <input class="form-control" placeholder="Enter youtube video URL" value="{$article['article']['cover_video']}" name="cover_video">
                      </div>
                    </div>
                    ...
                    <div class="form-group form-row">
                      ...
                      <div class="col-md-10">
                        <select class="form-control" name="type">
                          ...
                          <option value="1" {if $article['article']['type'] == '1'} selected {/if}>{__("Square Image")}</option>
                          <option value="2" {if $article['article']['type'] == '2'} selected {/if}>{__("Rectangular Image")}</option>
      					</select>
                      </div>
                    </div>
      			  <div class="form-group form-row">
                      ...
                      <div class="col-md-10">
                        <select class="form-control" name="category[]" multiple style="height:250px;">
                          ...
                          {foreach $blogs_categories as $category}
                            {include file='__categories.recursive_new_options.tpl' data_category=$article['article']['all_cats']}
                          {/foreach}
                        </select>
                      </div>
                    </div>
                  ...
                  <div class="card-body">
      			  ...
      			  <div class="form-group form-row">
                      ...
                      <div class="col-md-10">
                        <select class="form-control" name="type">
                          <option value="">{__("Select Image Type")}</option>
                          <option value="1">{__("Square Image")}</option>
                          <option value="2">{__("Rectangular Image")}</option>
                        </select>
                      </div>
                    </div>
