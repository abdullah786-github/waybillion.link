    ...
    <div class="col-12 col-md-10 mx-md-auto">
      <div class="card shadow">
        <div class="card-body text-center">
          ...
          <h2>{__("Hey")} 👋 {__("Complete your registration by")} <!-- {if $user->_data['registration_method'] == "ByEmail"}{__("Email")}{else}{__("Phone Number")}{/if}--> 
          </h2>
          <p class="text-xlg mt10 mb30">
            {if $user->_data['registration_method'] == "ByEmail"}
              {__("Please go to")} <span class="text-primary">{$user->_data['user_email']}</span> {__("to complete the activation process")}
            {else}
              {__("OTP has been sent to")} <strong>{$user->_data['user_phone']}</strong> <!-- {__("to complete the activation process")} -->
            {/if}
          </p>
