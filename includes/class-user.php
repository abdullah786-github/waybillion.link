...
class User
{
  ...
  public function article_search($query, $tab, $offset = 0)
  {
    global $db, $system;
    $results = [];
    if ($tab != "posts") {
    ...
    switch ($tab) {
      ...
      case 'articles':
        /* search articles */
        if ($system['blogs_enabled']) {
          $get_articles = $db->query(sprintf('SELECT post_id FROM posts_articles WHERE title LIKE %1$s OR text LIKE %1$s OR tags LIKE %1$s ORDER BY title ASC', secure($query, 'search'), secure($offset, 'int', false))) or _error("SQL_ERROR_THROWEN");
          if ($get_articles->num_rows > 0) {
            while ($article = $get_articles->fetch_assoc()) {
              $article = $this->get_post($article['post_id']);
              if ($article) {
  ...
  public function connect($do, $id, $uid = null)
  {
    ...
    switch ($do) {
      ...
      case 'friend-add':
        ...
        if($target_user['user_group']==2){

          /* add the friend request */
          $db->query(sprintf("INSERT INTO friends (user_one_id, user_two_id, status) VALUES (%s, %s, '1')", secure($this->_data['user_id'], 'int'),  secure($id, 'int'))) or _error("SQL_ERROR_THROWEN");
  ...
  public function get_trending_hashtags_page($id)
  {
    global $system, $db;
    $hashtags = [];
    $get_trending_hashtags = $db->query(sprintf("SELECT hashtags.hashtag, COUNT(hashtags_posts.id) AS frequency FROM hashtags LEFT JOIN hashtags_posts ON hashtags.hashtag_id = hashtags_posts.hashtag_id LEFT JOIN posts ON hashtags_posts.post_id = posts.post_id WHERE hashtags_posts.created_at > DATE_SUB(CURDATE(), INTERVAL 1 %s) AND posts.user_id = %s GROUP BY hashtags_posts.hashtag_id ORDER BY frequency DESC LIMIT %s", secure($system['trending_hashtags_interval'], "", false), secure($id, "int", false), secure($system['trending_hashtags_limit'], 'int', false))) or _error("SQL_ERROR_THROWEN");
    if ($get_trending_hashtags->num_rows > 0) {
      while ($hashtag = $get_trending_hashtags->fetch_assoc()) {
        $hashtag['hashtag'] = html_entity_decode($hashtag['hashtag'], ENT_QUOTES);
        $hashtags[] = $hashtag;
  ...
  public function get_posts($args = [])
  {
    ...
    switch ($get) {
      ...
      case 'popular':
		if (isset($args['id'])) {
			$id = $args['id'];
			$where_query .= "WHERE posts.privacy = 'public' AND posts.user_id = $id"; /* get all popular public posts */
	  ...
	  if (isset($args['id'])) {
		$id = $args['id'];
        $where_query .= sprintf("WHERE posts.privacy = 'public' AND posts.user_id = %s", secure($id, 'int'));
	  }else{
    ...
    if ($last_post_id != null && $get != 'popular' && $get != 'saved' && $get != 'memories') { /* excluded as not ordered by post_id */
      $get_posts = $db->query(sprintf("SELECT * FROM (SELECT posts.post_id FROM posts " . $location_join . $where_query . ") posts WHERE posts.post_id > %s ORDER BY posts.post_id DESC", secure($last_post_id, 'int'))) or _error("SQL_ERROR_THROWEN");
		...
		if(isset($args['my_limit'])){
			$limit_statement = ($get_all) ? "" : sprintf("LIMIT %s, %s", secure($offset, 'int', false), secure($args['my_limit'], 'int', false)); /* get_all for cases like download user's posts */
		}else{
  ...
  public function get_post($post_id, $get_comments = true, $pass_privacy_check = false)
  {
    ...
    switch ($post['post_type']) {
      ...
      case 'article':
		...
		$cats = $db->query("SELECT m.category_id, GROUP_CONCAT(c.category_name SEPARATOR ',') AS cats FROM posts_articles AS m, blogs_categories AS c WHERE FIND_IN_SET(c.category_id, m.category_id) != 0 AND m.post_id = ".$post['post_id']." GROUP BY m.category_id");
		$allcats = $cats->fetch_assoc();
		$ids = explode(",",$allcats['category_id']);
		$cats = explode(",",$allcats['cats']);
		$all_cats = array_combine($ids,$cats);
        /* check if article has been deleted */
        if ($get_article->num_rows == 0) {
          return false;
        }
        $post['article'] = $get_article->fetch_assoc();
        $post['article']['all_cats'] = $all_cats;
        $post['article']['type'] = $post['article']['type'];
  ...
  public function get_categories_page($table_name, $category_id = 0, $reverse = false, $only_parents = false, $page_id)
  {
    global $db;
    $tree = [];
    $utree = [];
    if (!$reverse) {
      // top-down tree (default)
      $page_categories = $db->query(sprintf("SELECT posts_articles.category_id FROM posts_articles LEFT JOIN posts ON posts.post_id = posts_articles.post_id WHERE posts.user_id = %s ", secure($page_id, 'int'))) or _error("SQL_ERROR_THROWEN");
	  $array = array();
	  if ($page_categories->num_rows > 0) {
        while ($category = $page_categories->fetch_assoc()) {
		$ex = explode(',',$category['category_id']);
		array_push($array,$ex);
		}
	  }
	  ...
	  foreach($array2 as $cat_id){
	 $get_categories = $db->query(sprintf("SELECT * FROM %s WHERE category_id = %s ORDER BY category_order ASC ", $table_name, secure($cat_id, 'int'))) or _error("SQL_ERROR_THROWEN");
      if ($get_categories->num_rows > 0) {
        while ($category = $get_categories->fetch_assoc()) {
          $category['category_url'] = get_url_text($category['category_name']);
          $category['category_name'] = html_entity_decode($category['category_name'], ENT_QUOTES);
	  ...
	  foreach($trees as $leaf){
		  $cat_id = $leaf['category_parent_id'];
		  if($cat_id !== '0'){
			$parent[] = $this->get_category($table_name, $cat_id, true);
		  }else{
			  $parent[] = $leaf;
		  }
	  }
  ...
  public function get_pages($args = [])
  {
    ...
    $pagesbox = !isset($args['page_id']) ? null : $args['page_id'];
    ...
    if ($promoted) {
      ...
      /* get pagesbox pages */
       $get_pages = $db->query(sprintf("SELECT * FROM pages  WHERE page_id != %s", secure($pagesbox, 'int'))) or _error("SQL_ERROR_THROWEN");
    } elseif ($boosted) {
  ...
  public function get_all_pages()
  {
    ...
    $get_pages = $db->query(sprintf("SELECT * FROM pages")) or _error("SQL_ERROR_THROWEN");
    if ($get_pages->num_rows > 0) {
      while ($page = $get_pages->fetch_assoc()) {
        $page['page_picture'] = get_picture($page['page_picture'], 'page');
        /* check if the viewer liked the page */
  ...
  public function edit_page($page_id, $edit, $args)
  {
    ...
    switch ($edit) {
      case 'settings':
        ...
        /* update page */
        $db->query(sprintf("UPDATE pages SET page_category = %s, page_name = %s, page_title = %s, page_verified = %s WHERE page_id = %s", secure($args['category']), secure($args['username']), secure($args['title']), secure($args['page_verified']), secure($page_id, 'int'))) or _error("SQL_ERROR_THROWEN");
        break;
  ...
  public function get_articles($args = [])
  {
    ...
    $offset *= $results;
    /* get posts */
    $where_query = ($category != null) ? sprintf("AND FIND_IN_SET('%s',posts_articles.category_id)>0 ", $args['category']) : "";
    $order_query = ($random) ? "ORDER BY RAND()" : "ORDER BY posts.post_id DESC";
	//echo "SELECT posts.post_id FROM posts INNER JOIN posts_articles ON posts.post_id = posts_articles.post_id LEFT JOIN users ON posts.user_id = users.user_id AND posts.user_type = 'user' LEFT JOIN pages ON posts.user_id = users.user_id AND posts.user_type = 'page' WHERE posts.in_group = '0' AND posts.in_event = '0' AND posts.post_type = 'article' " . $where_query . " " . $order_query . " LIMIT %s, %s", secure($offset, 'int', false), secure($results, 'int', false);
   ...
   * @param string $media_type
   * @param string $cover
   * @param string $cover_video
   * @param integer $category_id
   * @param string $tags
   * @return integer
   */
  public function post_article($publish_to, $page_id, $group_id, $event_id, $title, $text, $media_type, $cover, $cover_video, $category_id, $tags, $type)
  {
    global $db, $system, $date;
    ...
    /* insert article */
    $db->query(sprintf("INSERT INTO posts_articles (post_id, media_type, cover, cover_video, title, text, category_id, tags, type) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", secure($post_id, 'int'), secure($media_type), secure($cover), secure($cover_video), secure($title), secure($clean_text), secure($category_id), secure($tags), secure($type, 'int'))) or _error("SQL_ERROR_THROWEN");
    /* points balance */
   ...
   * @return void
   */
  public function edit_article($post_id, $title, $text, $media_type, $cover, $cover_video, $category_id, $tags, $type)
  {
    global $db, $system, $date;
    /* check if blogs enabled */
    if (!$system['blogs_enabled']) {
    ...
    /* update the article */
    $db->query(sprintf("UPDATE posts_articles SET media_type = %s, cover = %s, cover_video = %s, title = %s, text = %s, category_id = %s, tags = %s, type = %s WHERE post_id = %s", secure($media_type, 'int'), secure($cover), secure($cover_video), secure($title), secure($clean_text), secure($category_id), secure($tags), secure($type, 'int'), secure($post_id, 'int'))) or _error("SQL_ERROR_THROWEN");
  }
  ...
  public function offer_pay($amount)
  {
    global $db, $system;
    /* check if wallet enabled */
    if (!$system['wallet_enabled']) {
    ...
    /* decrease viewer user points balance */
    $balance = $this->_data['user_points'] - ($system['points_per_currency'] * $amount);
    $db->query(sprintf("UPDATE users SET user_points = %s WHERE user_id = %s", secure($balance), secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
    /* increase viewer user wallet balance */
    /* wallet transaction */
    $this->wallet_offer_pay($this->_data['user_id'], 'offer_pay', 0, $amount, 'out');
    $_SESSION['wallet_withdraw_points_amount'] = $amount;
  }
  
  public function wallet_offer_pay($user_id, $node_type, $node_id, $amount, $type)
  {
    global $db, $system, $date;
  ...
  public function settings($edit, $args)
  {
    ...
    switch ($edit) {
      ...
      case 'work':
        ...
        $this->set_custom_fields($args, "user", "settings", $this->_data['user_id']);
        /* update user */
        $db->query(sprintf("UPDATE users SET user_work_title = %s, user_work_place = %s, user_work_url = %s , interest = %s WHERE user_id = %s", secure($args['work_title']), secure($args['work_place']), secure($args['work_url']), secure($args['category']), secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
        break;
      ...
      case 'started':
		...
		if(isset($args['user_phone'])){
			$db->query(sprintf("UPDATE users SET user_country = %s, user_work_title = %s, user_work_place = %s, user_work_url = %s, user_current_city = %s, user_hometown = %s, user_edu_major = %s, user_edu_school = %s, user_edu_class = %s, user_firstname = %s, user_lastname = %s, user_name = %s, user_gender = %s, user_birthdate = %s, user_phone = %s WHERE user_id = %s", secure($args['country'], 'int'), secure($args['work_title']), secure($args['work_place']), secure($args['work_url']), secure($args['city']), secure($args['hometown']), secure($args['edu_major']), secure($args['edu_school']), secure($args['edu_class']), secure($args['user_firstname']), secure($args['user_lastname']), secure($args['user_name']), secure($args['user_gender']), secure($dob), secure($args['user_phone']), secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
		}else{
			$db->query(sprintf("UPDATE users SET user_country = %s, user_work_title = %s, user_work_place = %s, user_work_url = %s, user_current_city = %s, user_hometown = %s, user_edu_major = %s, user_edu_school = %s, user_edu_class = %s, user_firstname = %s, user_lastname = %s, user_name = %s, user_gender = %s, user_birthdate = %s WHERE user_id = %s", secure($args['country'], 'int'), secure($args['work_title']), secure($args['work_place']), secure($args['work_url']), secure($args['city']), secure($args['hometown']), secure($args['edu_major']), secure($args['edu_school']), secure($args['edu_class']), secure($args['user_firstname']), secure($args['user_lastname']), secure($args['user_name']), secure($args['user_gender']), secure($dob), secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
		}
        break;
    }
  }
  ...
  public function sign_up($args = [])
  {
	...
	if (is_empty($args['password']) &&  $args['registration_method'] == 'ByEmail') {
      throw new Exception(__("You must fill in all of the fields"));
    }
    if (!valid_username($args['username'])) {
      throw new Exception(__("Please enter a valid username (a-z0-9_.) with minimum 3 characters long"));
    ...
    if (!valid_email($args['email']) &&  $args['registration_method'] == 'ByEmail') {
      throw new Exception(__("Please enter a valid email address"));
    }
    if ($this->check_email($args['email']) &&  $args['registration_method'] == 'ByEmail') {
      throw new Exception(__("Sorry, it looks like") . " <strong>" . $args['email'] . "</strong> " . __("belongs to an existing account"));
    }
    if ($system['activation_enabled'] && $system['activation_type'] == "sms" &&  $args['registration_method'] == 'ByPhone') {
    ...
    if (strlen($args['password']) < 6  &&  $args['registration_method'] == 'ByEmail') {
    ...
    $email_verification_code = ($system['activation_enabled'] && $args['registration_method'] == "ByEmail") ? get_hash_token() : 'null';
    $phone_verification_code = ($system['activation_enabled'] && $args['registration_method'] == "ByPhone") ? get_hash_key(6, true) : 'null';
    /* register user */
     $pointvalue  = $db->query(sprintf("SELECT option_value as option_value FROM system_options WHERE option_id = %s",2463)) or _error("SQL_ERROR_THROWEN");
  
    $result = $pointvalue->fetch_assoc();
    $db->query(sprintf("INSERT INTO users (user_name, user_email, user_phone, user_password, user_firstname, user_lastname, user_gender, user_birthdate, user_registered, user_email_verification_code, user_phone_verification_code, registration_method,user_privacy_newsletter,user_points) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s)", secure($args['username']), secure($args['email']), secure($args['phone']), secure(_password_hash($args['password'])), secure(ucwords($args['first_name'])), secure(ucwords($args['last_name'])), secure($args['gender']), secure($args['birth_date']), secure($date), secure($email_verification_code), secure($phone_verification_code),secure($args['registration_method']), secure($newsletter_agree), $result['option_value'])) or _error("SQL_ERROR_THROWEN");
    /* get user_id */
    ...
    /* send activation */
    if ($system['activation_enabled']) {
      if ($args['registration_method'] == "ByEmail") {
  ...
  public function sign_in_phone($phone, $code,$otp, $remember = false)
  {
    global $db, $system, $date;
	$phone_number = $code.$phone;
	$check = $db->query(sprintf("SELECT * FROM login_with_phone WHERE phone_number = %s AND otp = %s", secure($phone_number), secure($otp))) or _error("SQL_ERROR_THROWEN");
	if ($check->num_rows > 0) {
  ...
  public function sign_in_send_otp($phone)
  {
    global $db, $system, $date;
	$phone_number = $phone;
	$check = $db->query(sprintf("SELECT * FROM users WHERE user_phone = %s", secure($phone_number))) or _error("SQL_ERROR_THROWEN");
	if ($check->num_rows > 0) {
		while ($users = $check->fetch_assoc()) {
			$user['user_id'] = $users['user_id'];
			$phone_verification_code = ($system['activation_enabled']) ? get_hash_key(6, true) : 'null';
			/* prepare activation SMS */
			$message  = __($system['system_title']) . " " . __("OTP for login") . ": " . $phone_verification_code;
			/* send SMS */
			if (!sms_send($phone_number, $message)) {
			  throw new Exception(__("OTP for login could not be sent") . ", " . __("But you can login now"));
			}
			$new = $db->query(sprintf("SELECT * FROM login_with_phone WHERE user_id = %s", secure($user['user_id']))) or _error("SQL_ERROR_THROWEN");
			if ($new->num_rows == 0) {
				$db->query(sprintf("INSERT INTO login_with_phone (user_id, phone_number, otp) VALUES (%s,%s,%s)", secure($user['user_id']), secure($phone_number), secure($phone_verification_code))) or _error("SQL_ERROR_THROWEN");
			}else{
				$db->query(sprintf("UPDATE login_with_phone SET phone_number = %s, otp = %s WHERE user_id = %s", secure($phone_number), secure($phone_verification_code), secure($user['user_id']))) or _error("SQL_ERROR_THROWEN");
			}
			return 1;
		...
		return 0;
		//throw new Exception(__("Wrong Phone number"));
	}
  ...
  public function socail_register($first_name, $last_name, $username, $email, $password, $gender, $newsletter_agree, $privacy_agree, $avatar, $provider, $invitation_code = "")
  {
    ...
    /* register user */
    $db->query(sprintf("INSERT INTO users (user_points,user_name, user_email, user_password, user_firstname, user_lastname, user_gender, user_registered, user_activated, user_picture, $social_id, $social_connected) VALUES (%s,%s, %s, %s, %s, %s, %s, %s, '1', %s, %s, '1')",$result['option_value'], secure($username), secure($email), secure(_password_hash($password)), secure(ucwords($first_name)), secure(ucwords($last_name)), secure($gender), secure($date), secure($image_name), secure($_SESSION['social_id']))) or _error("SQL_ERROR_THROWEN");
    /* get user_id */
    $user_id = $db->insert_id;
    /* affiliates system */
    $this->process_affiliates("registration", $user_id);
