...
function load_more(element) {
  ...
  let needLogin = false;
  let isLogin = 0;
  if(_this.data('is-login') !== undefined) {
    needLogin = true;
    isLogin = parseInt(_this.data('is-login'));
  }
...
$(function () {
      ...
      if (hashtags !== null && hashtags.length > 0) {
        ...
        $.post(api['data/search'], { 'query': query }, function (response) {
          if (response.callback) {
            ...
            $('#search-results-all').attr('href', site_path + '/search/' + query + '/articles');
          } else {
            $('#search-results .dropdown-widget-header').hide();
            $('#search-results-all').hide();
    ...
    if (event.keyCode == 13) {
      ...
      if (!is_empty(query)) {
        ...
        if (hashtags !== null && hashtags.length > 0) {
          ...
          window.location = site_path + '/search/' + query + '/articles'
        }
      }
      return false;
    }
  ...
  $('body').on('submit', '.article_search', function (e) {
    e.preventDefault;
    ...
    if (!is_empty(query)) {
      ...
      if (handle !== undefined) {
        window.location = site_path + '/' + handle + '/article_search/' + query + filter + '/articles';
      } else {
        var hashtags = query.match(/#(\w+)/ig);
        if (hashtags !== null && hashtags.length > 0) {
          var query = hashtags[0].replace("#", "");
          window.location = site_path + '/article_search/hashtag/' + query + '/articles';
        } else {
          window.location = site_path + '/article_search/' + query + filter + '/articles';
        }
      }
    }
    return false;
  ...
  $('body').on('submit', '.js_search-form', function (e) {
    ...
    if (!is_empty(query)) {
      var filter = ($(this).data('filter') !== undefined) ? '/' + $(this).data('filter') : '';
      if (handle !== undefined) {
        window.location = site_path + '/' + handle + '/search/' + query + filter + '/articles';
      } else {
        var hashtags = query.match(/#(\w+)/ig);
        if (hashtags !== null && hashtags.length > 0) {
          var query = hashtags[0].replace("#", "");
          window.location = site_path + '/search/hashtag/' + query + '/articles';
        } else {
          window.location = site_path + '/search/' + query + filter + '/articles';
        }
      }
    }
    return false;
